#!/usr/bin/env python3
"""
dorar_aqeeda_export.py — Export dorar.net/aqeeda → EPUB + Markdown
Usage:
    python dorar_aqeeda_export.py
    TEST_PAGES=10 python dorar_aqeeda_export.py
"""
...
REFS_URL    = "https://dorar.net/refs/aqeeda"
ARTICLE_URL = "https://dorar.net/article/1987"
PAGE_RE     = re.compile(r"/aqeeda/(\d+)")
SKIP_CRUMBS = 2          # الرئيسة + الموسوعة العقدية
DELAY       = 0.5
TIMEOUT     = 20
TEST_PAGES  = int(os.getenv("TEST_PAGES") or 0)
OUT_DIR     = Path("output")
EPUB_PATH   = OUT_DIR / "aqeeda.epub"
MD_DIR      = OUT_DIR / "md"
BOOK_TITLE  = "الموسوعة العقدية"
...
# هرم الموسوعة العقدية (بعد إزالة SKIP_CRUMBS)
...
GREEN_RE   = re.compile(r"color\s*:\s*#0{0,2}80{3,4}",            re.I)
BLUE_RE    = re.compile(r"color\s*:\s*#0{0,2}0{0,2}[Ff]{2}",     re.I)
CRIMSON_RE = re.compile(r"color\s*:\s*#([Bb]22222|[Cc][Cc]0{4})", re.I)
...
def _count_phrase(n: int, child_type: str) -> str:
    plurals_few = {
        "كتاب": "كتب",    "باب": "أبواب",   "فصل": "فصول",
        "مبحث": "مباحث",  "مطلب": "مطالب",  "فرع": "فروع",
    }
    plurals_many = {
        "كتاب": "كتاباً",  "باب": "باباً",   "فصل": "فصلاً",
        "مبحث": "مبحثاً",  "مطلب": "مطلباً", "فرع": "فرعاً",
    }
    if n == 1:
        ...
        return f"وفيه {n} {plurals_few.get(child_type, child_type + 'ات')}"
    else:
        return f"وفيه {n} {plurals_many.get(child_type, child_type)}"
...
def _extract_refs_content(soup: BeautifulSoup) -> str:
    ...
    for article in body.find_all("article"):
        ...
        out_parts.append(f"<p><strong>{title}</strong>{sep}{details_str}</p>")
...
def main() -> None:
    ...
    print(f"=== dorar_aqeeda_export  [{mode}] ===\n")
